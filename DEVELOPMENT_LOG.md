...
## 2026-08-14｜教师个人课表显示不可上课时段

- 相关 commit：`19759d3 feat: show teacher unavailability in personal timetables`。

### 功能与语义

- 教师个人课表会同时读取该教师已设置的不可上课时段，并在周课表中以黑色 `Unavailable` 区块明确标示。学生班级和教室个人课表不会混入教师窗口。
- 个人课表 API 以同一 SQLite 读快照返回课次和不可用时段；首次载入与五秒轮询都原子更新两组画面资料，切换对象或退出时会清除旧黑块。
- 黑色区块保持可读的无障碍标签，且不截获鼠标操作；如果不可用时段与已排课重叠，黑色覆盖仍会明确显示冲突。

### 验证

- TypeScript、ESLint、production build、完整 CRUD、个人课表 API 合同、跨进程并发、UX 静态门槛与性能回归均通过。
- 验收覆盖只返回目标教师窗口、非教师窗口为空、轮询同步、切换后清除旧状态，以及冲突课卡与黑色时段的显示层级。

## 2026-08-14｜开始新周期时重置教师与年级不可用时段

- 相关 commit：`f82a548 fix: reset availability when starting new cycles`。

### 功能与资料安全

- `Back up, clear availability and start new cycle` 现在先把课程、教学分配、班次、班次—学生班级关联、已排课次、全部教师不可上课时段及 Year 1–3 blocked times 写入同一份应急快照，再于同一 `IMMEDIATE` 事务内清空这些周期资料。
- 教师、学生班级、教室、规则设置和账号仍跨周期保留。备份、课程清空或不可用时段清空任何一步失败，整个操作都会回滚，不会留下半份备份或半清空状态。
- 周期版本指纹现包含两类不可用时段。如果另一位排课者在 Cycle 页打开后改了任何窗口，旧确认会以 409 拒绝并要求刷新，不会清除用户未看过的新资料。

### 恢复与兼容

- 恢复新格式应急备份时，会用快照中的课表和两类不可用时段完整替换当前周期，再依据恢复后的窗口重算课表警告。
- 旧版五数组应急备份仍可恢复课程与课表；因为旧格式从未保存不可用时段，恢复时会保留当前 live 窗口，避免默默删除无法从历史快照取回的限制。
- Cycle 页已明确显示清空和恢复范围，开始新周期需额外确认不可用时段将被重置。成功后页面会清除旧总表、个人课表和窗口缓存，并重新读取 Rules workspace。

### 验证

- TypeScript、完整 ESLint、Prisma validate/schema diff、UX 静态门槛、Next.js production build、完整 CRUD、双 standalone 跨进程并发及性能回归全部通过。
- CRUD 以非空 Teacher 和 Year 窗口验证：快照精确包含旧窗口、Start 后两表为空、Restore 覆盖新周期临时窗口并精确恢复快照；同时覆盖窗口清空失败全回滚、非法／重复快照拒绝和旧备份兼容。
- 两个独立服务同时 Start 或 Restore 时均严格只有一个 200 赢家和一个 409 败家；赢家备份、清空与恢复的两类窗口逐字段正确。

### 下一步

1. 后续每个可验证的功能批次继续在本日志记录功能、验证、commit 与待办。
2. 待 Railway 配置完成后，补记真实域名、持久卷、HTTPS、重启持久性和备份／恢复演练证据。
