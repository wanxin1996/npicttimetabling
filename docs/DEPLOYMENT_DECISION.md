...
项目负责人已确认首个正式试用环境采用 **Railway 单个 Next.js 服务 + SQLite 持久卷**。
...
**2026-08-10 已选择 Railway + SQLite 持久卷。** 当前开发和首轮真实多人试用均按路线 A 推进；路线 B 保留为规模或高可用要求增长后的升级方案。

仓库已包含 Railway 配置和持久存储启动保护。下一阶段是在 Railway 创建真实服务、挂载持久卷、取得公网域名，并完成备份恢复和双账号验收；在这些动作完成前，不上传本机正式数据库。
