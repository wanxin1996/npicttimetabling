...
4. 恢复演练必须在正式交付前完成：记录恢复前的教师、课程、班次和排课数量，恢复一个已知备份，部署 Railway staged changes，再核对数量与 `GET /api/health`。
5. Railway 恢复会建立替代卷并保留原卷但卸载；确认新卷正确前不要删除旧卷。
6. 删除 Volume 会一并删除其平台备份，因此系统下载的项目外副本不能省略。
